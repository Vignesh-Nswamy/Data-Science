...
reg_x = scaler.transform(test_data.iloc[:, reg_columns])
clas_x = scaler.transform(test_data.iloc[:, class_columns])
# test_data.head()
```

```{python}
test_data['Democratic'] = dem_model.predict(reg_x)
```

```{python}
test_data['Republican'] = rep_model.predict(reg_x)
```

```{python}
test_data['Party'] = nb_classifier.predict()
```
